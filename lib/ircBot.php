...
class ircBot
{
    ...
    function __destruct ()
    {
        $this->log("<hr><b>Ended: ".date('Y-m-d h:i:s')."</b>");
        $this->say($this->config['channel'], "{$this->config['nick']} Shutting Down!");
    ...
    private function main ()
    {
        ...
        while (true)
        {
            ...
            if ($msg[1] == 'PRIVMSG')
            {
                $this->log($this->filter_log($msg[1], $msg[2], $msg[0], $this->getMsg($msg, 3)));
    ...
    {
        if ($msg and $this->config['logging'])
        {
            $logfile = fopen(rtrim($this->config['log_path'],'/')."/log-".date("Ym").".html", "a");
     ...
     * =============================================================================================================
     */
    public function addParser ($rexp, $func)
    {
        if (is_string($func))
            echo "-- parser '{$rexp}' registered: {$func}\n";
        else
            echo "-- parser '{$rexp}' registered: {closure}\n";
        $this->parsers[] = array(
                                 'rexp' => $rexp,
                                 'func' => $func
                                );
     ...
     * =============================================================================================================
     */
    public function addTimer ($name = "", $secs = 0, $func)
    {
        if (!empty($name) and !empty($secs))
        {
            echo "-- timer registered: {$name} => {$secs}\n";
